...
async function exchangeRSA() {
    ...
    ws.send(JSON.stringify({ type: "publicKey", from: "B", key: ab2b64(pub) }));
    log("Crypto: RSA Public Key Sent to A", "crypto");
}
...
async function sendMessage() {
    ...
    if (!val) return;
    if (!aesKey) {
        ws.send(JSON.stringify({ type: "plain", from: "B", msg: val }));
        log("Me: " + val, "me");
    } else {
        const iv = crypto.getRandomValues(new Uint8Array(12));
        const enc = await crypto.subtle.encrypt({ name: "AES-GCM", iv }, aesKey, new TextEncoder().encode(val));
        ws.send(JSON.stringify({ type: "secure", from: "B", iv: ab2b64(iv), msg: ab2b64(enc) }));
...
async function handleMessage(event) {
    ...
    if (data.type === "publicKey") {
        ...
        log("Crypto: Received A's Public Key", "crypto");
        if (!rsaKeyPair) await exchangeRSA();
    } 
    else if (data.type === "aesKey") {
        const raw = await crypto.subtle.decrypt({ name: "RSA-OAEP" }, rsaKeyPair.privateKey, b642ab(data.key));
        aesKey = await crypto.subtle.importKey("raw", raw, { name: "AES-GCM" }, true, ["encrypt", "decrypt"]);
        log("Crypto: AES Key Decrypted. Secure chat ready!", "crypto");
    }
        ...
        log("A (Secure): " + new TextDecoder().decode(dec), "peer");
    }
}